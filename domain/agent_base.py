...
class AgentBase(ABC):
    ...
    def __init__(self, id, name, llm, memory: ShortTermMemory) -> None:
        self.id = id
        self.name = name
        self._llm = llm
...
tool_calls里的工具为并行使用，当参数值为"$ref:工具名#次数"时，有引用依赖的工具会在被引用工具完成后自动执行。
...
根据需求，决定下一步调用哪个工具。"""
    ...
    def _build_state_summary(self) -> str:
        ...
        prompt_fragment = self.memory.to_prompt()
        if prompt_fragment:
...
"""
    ...
    # 工具调用返回
    async def on_tool_call(self, tool_name: str, success: bool, respond: str,callBack=None) -> None:
        """
    ...
    def resolve_args(self, arguments: dict) -> dict:
        """将参数里的 $ref 占位符替换为实际值"""
        return {
            k: self._resolve_single_ref(v) if (
                isinstance(v, str) and bool(self._REF_PATTERN.fullmatch(v))
            ) else v

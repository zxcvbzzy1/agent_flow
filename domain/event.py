...
class WriteAgentEvent:
    """
    写作代理事件工厂。
    封装了内容创作流程（build_content_creation_graph）中的所有关键事件。
    使用静态方法模拟函数调用，方便 EDA 执行层快速构建事件。
    """
    name = "write_agent"
    # --- 1. 初始化阶段 ---
    @staticmethod
    def ProcessStart(**kwargs) -> Event:
        """会话开始，准备接收创作需求"""
        return Event("write.process.start", payload=kwargs)

    @staticmethod
    def RequirementsAnalysisRequested(**kwargs) -> Event:
        """触发需求分析节点"""
        return Event("requirements.analysis.requested", payload=kwargs)

    @staticmethod
    def RequirementsAnalysisDone(**kwargs) -> Event:
        """需求分析完成，产出结构化需求"""
        return Event("requirements.analysis.done", payload=kwargs)

    # --- 2. 规划与大纲阶段 ---
    @staticmethod
    def OutlineRequested(**kwargs) -> Event:
        """触发大纲生成节点（复杂任务）"""
        return Event("outline.requested", payload=kwargs)

    @staticmethod
    def OutlineDone(**kwargs) -> Event:
        """大纲生成完成"""
        return Event("outline.done", payload=kwargs)

    # --- 3. 初稿写作阶段 ---
    @staticmethod
    def FirstDraftRequested(**kwargs) -> Event:
        """触发初稿写作节点"""
        return Event("first.draft.requested", payload=kwargs)

    @staticmethod
    def FirstDraftDone(**kwargs) -> Event:
        """初稿写作完成"""
        return Event("first.draft.done", payload=kwargs)

    # --- 4. 自动评判阶段 ---
    @staticmethod
    def AutoJudgeRequested(**kwargs) -> Event:
        """触发自动评判节点"""
        return Event("auto.judge.requested", payload=kwargs)

    @staticmethod
    def AutoJudgeDone(**kwargs) -> Event:
        """自动评判完成，产出评分与反馈"""
        return Event("auto.judge.done", payload=kwargs)

    # --- 5. 决策与路由阶段 (Judge Decision) ---
    @staticmethod
    def StylePolishRequested(**kwargs) -> Event:
        """高分且需要润色时触发"""
        return Event("style.polish.requested", payload=kwargs)

    @staticmethod
    def StylePolishDone(**kwargs) -> Event:
        """风格润色完成"""
        return Event("style.polish.done", payload=kwargs)

    @staticmethod
    def RewriteRequested(**kwargs) -> Event:
        """低分时触发重写"""
        return Event("rewrite.requested", payload=kwargs)

    @staticmethod
    def RewriteDone(**kwargs) -> Event:
        """重写完成，准备重新评判"""
        return Event("rewrite.done", payload=kwargs)

    @staticmethod
    def ProcessAborted(**kwargs) -> Event:
        """达到重试上限或严重错误时触发中止"""
        return Event("process.aborted", payload=kwargs)

    # --- 6. 人工审核阶段 ---
    @staticmethod
    def HumanReviewRequested(**kwargs) -> Event:
        """触发人工审核节点（Checkpoint）"""
        return Event("human.review.requested", payload=kwargs)

    @staticmethod
    def HumanReviewDone(**kwargs) -> Event:
        """人工审核动作完成（等待决策）"""
        return Event("human.review.done", payload=kwargs)

    # --- 7. 人工决策与修改阶段 (Human Decision) ---
    @staticmethod
    def PublishRequested(**kwargs) -> Event:
        """人工审核通过，触发发布"""
        return Event("publish.requested", payload=kwargs)

    @staticmethod
    def PublishDone(**kwargs) -> Event:
        """发布完成"""
        return Event("publish.done", payload=kwargs)

    @staticmethod
    def HumanRevisionRequested(**kwargs) -> Event:
        """人工审核打回，触发修改"""
        return Event("human.revision.requested", payload=kwargs)

    @staticmethod
    def HumanRevisionDone(**kwargs) -> Event:
        """按人工意见修改完成，准备续写/重评"""
        return Event("human.revision.done", payload=kwargs)

    # --- 8. 流程结束 ---
    @staticmethod
    def ProcessEnded(**kwargs) -> Event:
        """流程正常结束"""
        return Event("write.process.ended", payload=kwargs)

class CriticAgentEvent:
    name = "critic_agent"
    ...
    def ProcessStart(**kwargs) -> Event:
        """会话开始，准备接收评价需求"""
        return Event("critic.process.start", payload=kwargs)
    
    @staticmethod
    def ProcessEnded(**kwargs) -> Event:
        """会话开始，准备接收评价需求"""
        return Event("critic.process.end", payload=kwargs)
    
    # --- 并行流程 ---
    @staticmethod
    def SummaryWriterOutput(**kwargs) -> Event:
        """总结写作完成"""
        return Event("critic.summary.writer.output", payload=kwargs)
    
    @staticmethod
    def AnalysisRequested(**kwargs) -> Event:
        """触发用户需求分析节点"""
        return Event("critic.analysis.requested", payload=kwargs)

    @staticmethod
    def AnalysisDone(**kwargs) -> Event:
        """用户需求分析完成，产出结构化需求"""
        return Event("critic.analysis.done", payload=kwargs)
    
    @staticmethod
    def SummaryWriterDone(**kwargs) -> Event:
        """总结写作完成"""
        return Event("critic.summary.writer.done", payload=kwargs)
    
    # --- 汇总流程 ---
    @staticmethod
    def SummaryRequested(**kwargs) -> Event:
        """触发总结节点"""
        return Event("critic.summary.requested", payload=kwargs)
    
    @staticmethod
    def SummaryRequestedDone(**kwargs) -> Event:
        """汇总完成"""
        return Event("critic.summary.done", payload=kwargs)

    # --- 评价流程 ---
    @staticmethod
    def CriticRequeste(**kwargs) -> Event:
        """触发评价节点"""
        return Event("critic.requested", payload=kwargs)
    
    @staticmethod
    def CriticDone(**kwargs) -> Event:
        """评价完成"""
        return Event("critic.done", payload=kwargs)
